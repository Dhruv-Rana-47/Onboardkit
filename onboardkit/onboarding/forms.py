...
from .models import OnboardingTemplate, TemplateSection, TemplateItem, UserTask,User,TemplateAssignment
...
class TemplateItemForm(forms.ModelForm):
    class Meta:
        model = TemplateItem
        fields = ['title', 'item_type', 'content', 'video_url', 'document',
              'expected_output', 'expected_duration_new', 'order']
...
class AssignTaskForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if not cleaned_data.get('template_item') and not cleaned_data.get('custom_task'):
            raise forms.ValidationError("You must select a template item or enter a custom task")
        return cleaned_data

class TaskFilterForm(forms.Form):
    status = forms.ChoiceField(
        choices=[('', 'All Statuses')] + list(UserTask.STATUS_CHOICES),  # Convert tuple to list
        required=False,
        label="Status"
    )
...
class TaskForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['template_item'].choices = [('', '---------')] + [(i.pk, str(i.title)) for i in items]
        ...
        if not (user.is_admin or (hasattr(self.instance, 'assigned_by') and user == self.instance.assigned_by)):
            self.fields.pop('status', None)

    class Meta:
        model = UserTask
        fields = ['user', 'template_item', 'custom_task', 'due_date', 'priority', 'status']
        widgets = {
            'due_date': forms.DateInput(attrs={'type': 'date'}),
